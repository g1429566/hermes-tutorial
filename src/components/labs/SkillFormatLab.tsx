...
import {
  SKILL_BODY_SECTIONS,
  SKILL_BODY_SECTIONS_EN,
  SKILL_EXAMPLE_YAML,
  SKILL_FORMAT_HOOK,
  SKILL_FORMAT_HOOK_EN,
  SKILL_FORMAT_INTRO,
  SKILL_FORMAT_INTRO_EN,
  SKILL_FORMAT_UI,
  SKILL_FRONTMATTER_FIELDS,
  SKILL_FRONTMATTER_FIELDS_EN,
  SKILL_LOAD_STEPS,
  SKILL_LOAD_STEPS_EN,
} from '@/data/skills';
...
export default function SkillFormatLab() {
  const { lang } = useLang();
  const intro = lang === 'en' ? SKILL_FORMAT_INTRO_EN : SKILL_FORMAT_INTRO;
  const fields = lang === 'en' ? SKILL_FRONTMATTER_FIELDS_EN : SKILL_FRONTMATTER_FIELDS;
  const bodySections = lang === 'en' ? SKILL_BODY_SECTIONS_EN : SKILL_BODY_SECTIONS;
  const loadSteps = lang === 'en' ? SKILL_LOAD_STEPS_EN : SKILL_LOAD_STEPS;
  const hook = lang === 'en' ? SKILL_FORMAT_HOOK_EN : SKILL_FORMAT_HOOK;
  ...
  const step = loadSteps.find((s) => s.id === stepId) ?? loadSteps[0];
  const idx = loadSteps.findIndex((s) => s.id === step.id);
  ...
  return (
    <section className="mt-10">
      ...
      <SectionHeading kicker="Frontmatter" title={pick(lang, SKILL_FORMAT_UI.frontmatterTitle)} />
      <div className="mt-6">
        <Explorer
          items={fields.map((f) => ({
            id: f.id,
        ...
        >
          <div className="rounded-lg border border-line bg-white p-6">
            <div className="flex flex-wrap items-baseline gap-3">
              ...
              >
                {field.required
                  ? pick(lang, SKILL_FORMAT_UI.required)
                  : pick(lang, SKILL_FORMAT_UI.optional)}
              </span>
            </div>
            <p className="mt-4 leading-relaxed text-ink/75">{field.purpose}</p>
            <h4 className="mt-6 font-mono text-xs tracking-[0.15em] text-muted">
              {pick(lang, SKILL_FORMAT_UI.rulesHeading)}
            </h4>
            ...
            <div className="mt-6">
              <CodeBlock
                file="SKILL.md"
                code={field.example}
                note={pick(lang, SKILL_FORMAT_UI.exampleNote)}
              />
            </div>
          </div>
        </Explorer>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div>
          <h4 className="font-mono text-xs tracking-[0.15em] text-muted">
            {pick(lang, SKILL_FORMAT_UI.realFrontmatterHeading)}
          </h4>
          <div className="mt-3">
        ...
        <div>
          <h4 className="font-mono text-xs tracking-[0.15em] text-muted">
            {pick(lang, SKILL_FORMAT_UI.bodySpecHeading)}
          </h4>
          <ul className="mt-3 space-y-2 rounded-lg border border-line bg-white p-5">
            ...
            <li className="pt-1 text-xs text-muted">{pick(lang, SKILL_FORMAT_UI.bodySpecNote)}</li>
          </ul>
        </div>
      </div>

      <SectionHeading
        kicker={pick(lang, SKILL_FORMAT_UI.loadKicker)}
        title={pick(lang, SKILL_FORMAT_UI.loadTitle)}
      />
      <div className="mt-6">
        <Stepper
          steps={loadSteps.map((s) => ({ id: s.id, label: s.label }))}
          current={step.id}
          onChange={selectStep}
        />
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              disabled={idx === 0}
              onClick={() => selectStep(loadSteps[idx - 1].id)}
            ...
            <button
              type="button"
              disabled={idx === loadSteps.length - 1}
              onClick={() => selectStep(loadSteps[idx + 1].id)}
              className="rounded border border-line px-3 py-1.5 font-mono text-xs text-muted hover:border-ink hover:text-ink disabled:opacity-40"
            >
              {t(lang, 'nextStep')}
      ...
      <SectionHeading
        kicker={pick(lang, SKILL_FORMAT_UI.hookKicker)}
        title={pick(lang, SKILL_FORMAT_UI.hookTitle)}
      />
