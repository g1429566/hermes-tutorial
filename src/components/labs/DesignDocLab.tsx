...
import {
  ...
  DESIGN_DOC_INTRO_EN,
  DESIGN_DOC_SECTIONS,
  DESIGN_DOC_SECTIONS_EN,
  DESIGN_DOC_UI,
} from '@/data/design-doc';
...
export default function DesignDocLab() {
  const { lang } = useLang();
  const intro = lang === 'en' ? DESIGN_DOC_INTRO_EN : DESIGN_DOC_INTRO;
  const docSections = lang === 'en' ? DESIGN_DOC_SECTIONS_EN : DESIGN_DOC_SECTIONS;
  ...
  const filled = docSections.filter((s) => (sections[s.id] ?? '').trim().length > 0).length;
      ...
      <div className="mt-8 max-w-3xl rounded-lg border border-line bg-white p-5">
        <div className="flex items-baseline justify-between">
          <p className="font-mono text-[11px] tracking-[0.15em] text-muted">
            {pick(lang, DESIGN_DOC_UI.completionLabel)}
          </p>
          <p className="font-mono text-sm">
            <span className={filled === total ? 'text-green' : 'text-ink'}>{filled}</span>
            <span className="text-muted">
              /{total} {pick(lang, DESIGN_DOC_UI.filledSuffix)}
            </span>
          </p>
        </div>
            ...
            <div key={s.id} className="rounded-lg border border-line bg-white p-6">
              <div className="flex items-baseline gap-3">
                ...
                {hasContent && (
                  <span className="font-mono text-xs text-green">
                    {pick(lang, DESIGN_DOC_UI.filled)}
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border border-line bg-paper-deep p-4">
                  <p className="font-mono text-[11px] tracking-[0.15em] text-muted">
                    {pick(lang, DESIGN_DOC_UI.interviewerWants)}
                  </p>
                ...
                <div className="rounded-lg border border-ink/20 bg-code-bg p-4">
                  <p className="font-mono text-[11px] tracking-[0.15em] text-acid">
                    {pick(lang, DESIGN_DOC_UI.hermesExample)}
                  </p>
              ...
              <details className="group mt-4">
                <summary className="cursor-pointer list-none rounded border border-dashed border-line px-4 py-2.5 font-mono text-xs text-muted transition-colors hover:border-ink hover:text-ink">
                  <span className="group-open:hidden">{pick(lang, DESIGN_DOC_UI.expand)}</span>
                  <span className="hidden group-open:inline">
                    {pick(lang, DESIGN_DOC_UI.collapse)}
                  </span>
                </summary>
                <textarea
                  value={sections[s.id] ?? ''}
