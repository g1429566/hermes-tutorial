...
import { AGENT_LOOP_INTRO, LOOP_STEPS } from '@/data/agent-loop';
...
export default function AgentLoopLab() {
  ...
  const step = LOOP_STEPS.find((s) => s.id === stepId) ?? LOOP_STEPS[0];
  const idx = LOOP_STEPS.findIndex((s) => s.id === step.id);
  ...
  return (
    <section className="mt-10">
      <p className="max-w-3xl leading-relaxed text-ink/75">{AGENT_LOOP_INTRO}</p>

      <div className="mt-8">
        <Stepper
          steps={LOOP_STEPS.map((s) => ({ id: s.id, label: s.label }))}
          current={step.id}
          onChange={select}
        />
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              disabled={idx === 0}
              onClick={() => select(LOOP_STEPS[idx - 1].id)}
            ...
            <button
              type="button"
              disabled={idx === LOOP_STEPS.length - 1}
              onClick={() => select(LOOP_STEPS[idx + 1].id)}
              className="rounded border border-line px-3 py-1.5 font-mono text-xs text-muted hover:border-ink hover:text-ink disabled:opacity-40"
            >
              下一步 ›
      ...
      <SectionHeading kicker="记忆钩子" title="一句话记住主循环" />
      <p className="mt-3 max-w-3xl rounded-lg border border-acid bg-acid/10 p-5 font-mono text-sm leading-relaxed">
        while 预算未耗尽：response = model(messages, tools)；有 tool_calls 就执行并 append
        结果继续循环，没有就 return content。——追加不改写，缓存不失效。
      </p>
    </section>
  );
}
