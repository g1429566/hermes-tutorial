...
import { HERO, TERMINAL_LINES, type TerminalLineKind } from '@/data/hero';
...
function HeroTerminal() {
  ...
  for (let i = 0; i < lineIdx && i < TERMINAL_LINES.length; i++) {
    visible.push({ ...TERMINAL_LINES[i], partial: false });
  }
  if (lineIdx < TERMINAL_LINES.length) {
    const line = TERMINAL_LINES[lineIdx];
    visible.push({
